...
A portable bash script for zero-friction setup. Run once to download Cicadas, check Python 3.11+, initialize `.cicadas/`, and optionally wire up agent integrations (`claude-code`, `antigravity`, `cursor`, `rovodev`, `codex`). Initialization is idempotent and preserves existing registry, index, config, and active state on rerun.
