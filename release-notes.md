...
- **Fix**: `gavel oneshot analyze` computed throughput from the raw spread of `OutputRecord.timestamp` values, which inflated results by orders of magnitude for concurrent runs — records finish and get spooled to `results_raw.jsonl` within milliseconds of each other regardless of how long each scenario actually took. `_compute_throughput()` now reconstructs each record's start time (`timestamp - timing_ms`) and measures the true execution window. Also, `RunMetrics` previously counted WARNING-tier (`PROCESS_SUCCESS_WITH_ISSUE`) outcomes the same as hard ERROR-tier failures in `error_rate`; a new `warning_count` field (shown as its own "Warnings" row in the CLI table) now tracks these separately. Found via a live end-to-end test running `ScriptInputProcessor` against a real local Ollama model.
