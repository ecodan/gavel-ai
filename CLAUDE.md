...
- **`gavel oneshot analyze`**: computes performance metrics (success/error counts, error rate, latency avg/p50/p95, throughput, token totals) from `results_raw.jsonl` via `core/run_metrics.py::compute_run_metrics()` — a pure function reading `OutputRecord` fields directly. Transport-agnostic — works for prompt-based and external runs alike.
