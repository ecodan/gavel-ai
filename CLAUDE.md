...
- **Closed-box (external SUT) evals**: `test_subject_type: "external"` is a config-driven target inside `oneshot` (`create/run/judge/analyze/report`), not a separate command group. `gavel oneshot create --type external` scaffolds both transport variants: `scripts/sut_script_scaffold.py` (active, `test_subjects[0]`, `system_id: "sut-script"`) and `scripts/sut_http_scaffold.py` (inert placeholder, `system_id: "sut-http"`, `judges: []`). `ScenarioProcessorStep` reads only `test_subjects[0]` for SUT execution, but `JudgeRunnerStep` pools `judges` across every `test_subjects` entry — keep `judges: []` on any inactive subject or it silently double-judges. External payloads (`scenario_input`, `rendered_prompt`, `custom_config`) must satisfy `ExternalTaskRequest`; `rendered_prompt` falls back to `str(scenario.input)` since external subjects have no prompt template.
- **`gavel oneshot analyze`**: computes performance metrics (success/warning/error counts, error rate, latency avg/p50/p95, throughput, token totals) from `results_raw.jsonl` via `core/run_metrics.py::compute_run_metrics()` — a pure function reading `OutputRecord` fields directly. Transport-agnostic — works for prompt-based and external runs alike. WARNING-tier outcomes (`metadata["external_tier"] == "WARNING"`) are tracked separately from ERROR-tier failures and excluded from `error_rate`. Throughput is derived from each record's reconstructed execution window (`timestamp - timing_ms`), not the raw spread of completion timestamps — concurrent scenarios spool to `results_raw.jsonl` within milliseconds of each other regardless of actual duration.
...
- [External Runner Schemas](docs/specs/schema-external-runner.md) — `ExternalTaskRequest`/`ExternalResponseEnvelope` payloads for the http/script transports, plus `oneshot create --type external` scaffolding and `oneshot analyze`
