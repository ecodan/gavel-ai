...
---
summary: "Nine targeted changes to the OneShot workflow. Implemented: DeterministicMetric hierarchy (ClassifierMetric + RegressionMetric, not Judge subclasses), TOML judge config resolution via config_ref, typed scaffolding templates, prompt snapshot, expanded validator, .workflow_status step tracking, score-averaging exclusion, deterministic report section, in-situ scaffold fix. Key divergences from original spec: DeterministicRunResult uses 'samples' (not 'per_sample'), 'judge_type' field added, prediction/actual are Optional[str]. OTel spans deferred. skipped_counts injected outside ReportData model."
phase: "tech"
when_to_load:
...
└── unit/
    └── judges/
        └── test_deterministic_metric.py  # NEW: unit tests for ClassifierMetric, RegressionMetric
```
...
**Decision:** `ReportRunnerStep` adds `processor_results` to `RunData` as a new field (`raw_results: List[Dict]`). `OneShotReporter._build_context()` builds a `processor_errors: Set[Tuple[str, str]]` (keyed by `(scenario_id, variant_id)`) from `raw_results`. When accumulating judge scores in the summary metrics loop, it skips entries where the key is in the error set. A `skipped_counts: Dict[str, Dict[str, int]]` dict tracks exclusions per `(variant, judge)` for report display.

**Implementation note:** `skipped_counts` is injected directly into the Jinja2 template context dict (outside the `ReportData` model) as `ctx["skipped_counts"] = skipped_counts`. This is a known divergence from the clean model→dict pattern — a follow-up can move it into `ReportData` or a companion dataclass.
...
class PerSampleDeterministicResult(BaseModel):
    ...
    scenario_id: str
    prediction: Optional[str] = None   # None when sample is skipped before extraction
    actual: Optional[str] = None       # None when sample is skipped before extraction
...
class DeterministicRunResult(BaseModel):
    ...
    model_config = ConfigDict(extra="ignore")
    metric_name: str
    judge_type: str                    # "classifier" or "regression" — from JudgeConfig.type
    report_metric: str
...
**Deferred.** OTel instrumentation for `DeterministicMetric.evaluate_sample()` was not implemented in this initiative. Intended design: emit a span using `self.tracer` (inherited from base class init via `get_tracer(__name__)`). Span name: `"deterministic_metric.evaluate_sample"`. Attributes: `metric.name`, `metric.type`, `scenario.id`, `metric.skipped` (bool). Track as a follow-up task.
...
1. **Foundation — Data models** *(blocking)*
