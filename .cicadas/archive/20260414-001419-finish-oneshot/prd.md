...
A CI run is killed mid-way (OOM). The operator opens the run artifact directory and reads `.workflow_status`. They can see that `process` completed but `judge` did not — giving them an exact diagnosis of where the failure occurred. They re-trigger the job; the re-run restarts from scratch (step-resumption is post-MVP), but the operator now knows which step to investigate rather than searching logs blind.

**Requirements Revealed:** `.workflow_status` step tracking, diagnostic artifact for interrupted runs.
...
**FR-1.5:** `JudgeRunnerStep` must run deterministic metrics in a synchronous inline loop (separate from `JudgeExecutor`): call `evaluate_sample()` per record in scenario order, then call `compute()` once after all samples to produce the population result. `JudgeExecutor` is unchanged.
