...
- **Judge score scale**: `JudgeResult`/`JudgeEvaluation`/`JudgedRecord.score` are `float, 0.0-1.0` — the native scale of every judge type (DeepEval and external/custom, both clamped to [0.0, 1.0]). Not a manufactured 1-10 integer.
- **Autotune scoring**: All `avg_score`/`convergence_threshold`/`target_score`/`degradation_tolerance` values are on the same **0.0–1.0** scale as `JudgeResult.score`, so `normalize_score()` is a pass-through. Convergence checked in order: `max_rounds_reached` → `target_score_achieved` → `minimal_improvement` → `performance_degraded`.
